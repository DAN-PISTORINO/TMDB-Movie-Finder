...
struct Node {
    ...
    Node* parent;
    bool isRed;
    Node(int year, string movie, string category, string movieID) : year(year), movie(movie), category(category), movieID(movieID),isRed(true),left(nullptr), right(nullptr), parent(nullptr){}
    ~Node() {
        left = nullptr;
        right = nullptr;
        delete left;
...
class RedBlackTree{
        ...
        Node* removeHelper(Node* &head, int &year, string &movie, string &category, string &movieID);

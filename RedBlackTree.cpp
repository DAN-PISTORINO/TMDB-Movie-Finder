...
void RedBlackTree::insert(int &year, string &movie, string &actor, string &category, string &movieID) {
    this->root = insertHelper(this->root, year, movie, actor, category, movieID);
    this->root->isRed = false;
}

//O(log(n))
Node* RedBlackTree::insertHelper(Node* &head, int &year, string &movie, string &actor, string &category, string &movieID) {
    //If the head is null, create a new node and return it
    if (head == nullptr) {
        head = new Node(year, movie, actor, category, movieID);
        return head;
    }

    //If the current year is less or  the current year is the same but the movie is less than the current movie, go left
    if (movieID < head->movieID || (movieID == head->movieID && movieID < head->movieID)) {
        head->left = insertHelper(head->left, year, movie, actor, category, movieID);
    ...
    //If the current year is greater or the current year is the same but the movie is greater than the current movie, go right
        ...
        head->right = insertHelper(head->right, year, movie, actor, category, movieID);
...
void RedBlackTree::removeNode(int &year, string &movie, string &actor, string &category, string &movieID) {
    this->root = removeHelper(this->root, year, movie, actor, category, movieID);
...
//O(log(n))
Node* RedBlackTree::removeHelper(Node* &head, int &year, string &movie, string &actor, string &category, string &movieID) {
    ...
    if (year < head->year || (year == head->year && movie < head->movie)) {
        head->left = removeHelper(head->left, year, movie, actor, category, movieID);
    } 

    else if (year > head->year || (year == head->year && movie > head->movie)) {
        head->right = removeHelper(head->right, year, movie, actor, category, movieID);
    } 
        ...
        //If 2 children
            ...
            head->right = removeHelper(head->right, successor->year, successor->movie, successor->actor, successor->category, successor->movieID);
        }
    }

...
Node* SplayTree::insertHelper(Node* &head, int &year, string &movie, string &category, string &movieID) {
    if (head == nullptr) {
        return new Node(year, movie, category, movieID);
    }
    ...
    Node* newnode = new Node(year, movie, category, movieID);

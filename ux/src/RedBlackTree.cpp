...
void RedBlackTree::insert(int &year, string &movie, string &actor, string &category) {
    this->root = insertHelper(this->root, year, movie, actor, category);
    this->root->isRed = false;
}

Node* RedBlackTree::insertHelper(Node* &head, int &year, string &movie, string &actor, string &category) {
    //If the head is null, create a new node and return it
    if (head == nullptr) {
        head = new Node(year, movie, actor, category);
        return head;
    }
    ...
    if (year < head->year || (year == head->year && movie < head->movie)) {
        head->left = insertHelper(head->left, year, movie, actor, category);
    ...
    //If the current year is greater or the current year is the same but the movie is greater than the current movie, go right
        ...
        head->right = insertHelper(head->right, year, movie, actor, category);
...
void RedBlackTree::balanceTree(Node* &head) {
    while (head != root && head->parent->isRed) {
        ...
        if (parent == grandparent->left) {
            ...
            if (uncle != nullptr && uncle->isRed) {
                ...
                if (head == parent->right) {
                    head = parent;
                    leftRotate(head);
                    parent = head->parent; // Update parent after rotation
                }
            ...
            // Symmetric case (right child of its parent)
...
void RedBlackTree::removeNode(int &year, string &movie, string &actor, string &category) {
...
Node* RedBlackTree::removeHelper(Node* &head, int &year, string &movie, string &actor, string &category) {
    ...
    if (year < head->year || (year == head->year && movie < head->movie)) {
        head->left = removeHelper(head->left, year, movie, actor, category);
    } 

    else if (year > head->year || (year == head->year && movie > head->movie)) {
        head->right = removeHelper(head->right, year, movie, actor, category);
    } 
        ...
        //If 2 children
            ...
            head->actor = successor->actor;
            head->category = successor->category;

            // Recursively remove the successor node
            head->right = removeHelper(head->right, successor->year, successor->movie, successor->actor, successor->category);
        }
    }
...
void RedBlackTree::searchHelper(Node* head, vector<string> &movies, string &category) {
...
void RedBlackTree::search(vector<string> &movies, string &category) {
